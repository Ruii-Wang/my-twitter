...
SECRET_KEY = '_%5sbnn97_d*1z9ogh#ktz2ca)$q%3$l*tsn-mo7cgafleky$8'
...
# 当用s3boto3作为用户上传文件存储时，需要按照在AWS上创建的配置来设置你的BUCKET_NAME
# 和REGION_NAME，这个值可以改成你自己创建的bucket的名字和所在的region
AWS_STORAGE_BUCKET_NAME = 'django-twitter-rw'
...
# use 'sudo apt-get install memcached'
...
# Then install the python connection of redis: pip install redis
...
RATELIMIT_CACHE_PREFIX = 'rl:' # 避免和其他的key冲突
RATELIMIT_ENABLE = not TESTING # 在某些环境下，比如内部测试等环境下，一般会关掉
