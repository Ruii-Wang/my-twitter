...
def required_params(request_attr='query_params', params=None):
    ...
    这个 decorator 函数的参数就是被 @required_params 包裹起来的函数 view_func
    """

    # 从效果上来说，参数中写 params = [] 很多时候也没有太大问题
    # 但是从编程习惯上说，函数的参数列表中的值不能是一个 mutable 的参数
    if params is None:
        params = []

    def decorator(view_func):
        """
        decorator 函数通过wraps将view_func里的参数解析出来传递给 _wrapped_view
        ...
        def _wrapped_view(instance, request, *args, **kwargs):
            ...
            # 完成检测之后，再调用被 @required_params包裹起来的view_func
